...
def get(i,d,h):
    H = Herbie(f"2025-03-{d} {i}:00", model="gfs", fxx=h)
    ds = H.xarray(f":TMP:2 m above ground:{h} hour fcst")
    return ds, H
...
def plot(ds, H, h):
    ...
    ax.set_title(f"Mid-Atlantic US // GFS // Init @ 12:00Z 26 Mar 2025\n[{h}h] // Valid @ {ds.valid_time.dt.strftime('%H:%MZ %d %b %Y').item()}", 
                loc="left", 
                color="#aab9d1", 
                size=11)
...
frames = []
for h in range(60,90,3):
    name = aniplot(12,26,h)
    frames.append(name)
...
# Output GIF path
output_gif_path = "gifs/weekendtmp_euro.gif"
# Create GIF
